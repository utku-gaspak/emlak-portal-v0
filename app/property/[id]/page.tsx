...
function SpecificationItem({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-3xl bg-white p-4 shadow-[0_12px_30px_rgba(15,23,42,0.06)]">
...
export default async function PropertyDetailPage({ params }: PropertyDetailPageProps) {
  ...
  return (
    <div data-automation="property-detail-container" className="mx-auto max-w-5xl space-y-8">
      <section className="overflow-hidden rounded-[2.5rem] bg-white shadow-[0_22px_70px_rgba(15,23,42,0.08)]">
        <div className="grid gap-8 p-6 sm:p-8 lg:grid-cols-[1.15fr_0.85fr] lg:p-10">
          <div className="space-y-6">
            <div className="inline-flex rounded-full bg-brand-50 px-4 py-1 text-xs font-semibold uppercase tracking-[0.22em] text-brand-700">
              {listing.type === "house" ? t.propertyDetail.houseTypeBadge : t.propertyDetail.landTypeBadge}
            </div>

            <div className="space-y-3">
              <h1 id="detail-title" data-automation="detail-title" className="text-4xl font-black tracking-tight text-slate-950 sm:text-5xl">
                {listing.title}
              </h1>
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div className="rounded-3xl bg-slate-50 p-5">
                <p className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-500">{t.propertyDetail.areaLabel}</p>
                <p className="mt-2 text-lg font-bold text-slate-900">
                  {listing.areaSqm} {t.common.squareMetersUnit}
                </p>
              </div>
              <div className="rounded-3xl bg-slate-50 p-5">
                <p className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-500">{t.propertyDetail.photosLabel}</p>
                <p className="mt-2 text-lg font-bold text-slate-900">{listing.images.length}</p>
              </div>
            </div>
          </div>

          <div className="relative overflow-hidden rounded-[2rem] bg-slate-100 shadow-[0_20px_60px_rgba(15,23,42,0.12)]">
      ...
      <section className="space-y-5 rounded-[2.5rem] bg-white p-6 shadow-[0_22px_70px_rgba(15,23,42,0.06)] sm:p-8">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <h2 className="mt-1 text-2xl font-black tracking-tight text-slate-950">{t.propertyDetail.galleryTitle}</h2>
          </div>
      ...
      <section className="rounded-[2.5rem] bg-slate-950 px-6 py-8 text-white shadow-[0_22px_70px_rgba(15,23,42,0.2)] sm:px-8">
        <p className="text-sm font-semibold uppercase tracking-[0.22em] text-slate-300">{t.propertyDetail.specifications}</p>
      ...
      <section className="grid gap-6 lg:grid-cols-[0.95fr_1.05fr]">
        <div className="rounded-[2.5rem] bg-white p-6 shadow-[0_22px_70px_rgba(15,23,42,0.06)]">
          <p className="text-sm font-semibold uppercase tracking-[0.2em] text-brand-700">{t.propertyDetail.overview}</p>
          <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div className="rounded-3xl bg-slate-50 p-5">
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">{t.propertyDetail.locationLabel}</p>
              <p className="mt-2 text-lg font-bold text-slate-900">{listing.location}</p>
            </div>
            <div className="rounded-3xl bg-slate-50 p-5">
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">{t.propertyDetail.areaLabel}</p>
              <p className="mt-2 text-lg font-bold text-slate-900">{listing.areaSqm}</p>
            </div>
          </div>
        </div>

        <aside className="rounded-[2.5rem] bg-white p-6 shadow-[0_22px_70px_rgba(15,23,42,0.06)]">
          <h2 className="text-sm font-semibold uppercase tracking-[0.2em] text-brand-700">{t.propertyDetail.description}</h2>
          <p id="detail-description" data-automation="detail-description" className="mt-4 whitespace-pre-line text-base leading-8 text-slate-700">
            {listing.description}
          </p>
        </aside>
      </section>
